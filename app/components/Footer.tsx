    ...
    <footer className="border-t" style={{ borderColor: "#2a2a2a", backgroundColor: "#0f0f0f" }}>
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <nav className="flex flex-wrap items-center gap-6">
            {links.map((link) => (
              <Link key={link.href} href={link.href} className="transition-colors" style={{ fontSize: "15px", color: "#b8b8b8" }}>
                {link.label}
              </Link>
            ))}
          </nav>
      ...
      <div className="border-t" style={{ borderColor: "#2a2a2a" }}>
        <div className="max-w-6xl mx-auto px-6 py-6">
          <p style={{ fontSize: "13px", color: "#b8b8b8" }}>&copy; 2026 Evergroup FZCO. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
