    ...
    <section className="py-20 sm:py-24 lg:py-32 border-t" style={{ borderColor: "#2a2a2a", backgroundColor: "#0f0f0f" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <ScrollAnimation delay={0.1}>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-10 mt-10 sm:mt-12">
            {stats.map((stat, index) => (
              <div
                key={stat.label}
                className={index > 0 ? "lg:border-l lg:pl-8" : ""}
                style={index > 0 ? { borderColor: "#2a2a2a" } : undefined}
              >
                <p style={{ fontSize: "clamp(28px, 5vw, 48px)", fontWeight: 700, fontFamily: "var(--font-display)", lineHeight: 1, color: "#ffffff" }}>
                  {stat.value}
                </p>
                <p className="mt-3" style={{ fontSize: "15px", fontWeight: 500, lineHeight: 1.6, color: "#999999" }}>{stat.label}</p>
